# ruff: noqa: N806, SIM210, RUF005, E501
...
if is_mamba_2_ssm_available():
    ...
    mamba_chunk_scan_combined, mamba_split_conv1d_scan_combined, selective_state_update = (
        None,
        None,
        None,
    )
...
class NemotronHMamba2Mixer(nn.Module):
    ...
    # fmt: off
    def torch_forward(self, input_states, cache_params: HybridMambaAttentionDynamicCache | None=None, cache_position:torch.LongTensor | None=None, attention_mask: torch.Tensor | None=None):
        ...
        if cache_params is not None and cache_position is not None and cache_position[0] > 0:
            ...
            ssm_states = cache_params.ssm_states[self.layer_idx].to(  # Shape: [b, h, d, n]
                device=C.device, dtype=C.dtype
            )
            # Reshape ssm_states to merge the first two dimensions
            ssm_states_reshaped = ssm_states.view(  # Shape: [b*h, d, n]
...
class NemotronHSdpaAttention(NemotronHAttention):
    ...
    def forward(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "NemotronHModel is using NemotronHSdpaAttention, but "
                "`torch.nn.functional.scaled_dot_product_attention` does not support "
                "`output_attentions=True`. Falling back to manual implementation."
            )
...
class NemotronHPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if self.config.rescale_prenorm_residual:
            ...
            #   > residual path with model depth. Scale weights by 1/√N.
...
NEMOTRONH_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `(batch_size, input_ids_length)`, *optional*):
            ...
            If `cache_params.seqlen_offset>0`, only `input_ids` without past should be passed as
            `input_ids`.
...
@add_start_docstrings(
    """
    The NEMOTRONH Model transformer with a language modeling head on top (linear layer
    with weights not tied to the input
    embeddings).
    """,
    NEMOTRONH_START_DOCSTRING,
)
        ...
        **kwargs,
        ...
        if inputs_embeds is not None and empty_past_kv:
            ...
            if input_ids is not None and inputs_embeds.shape[1] < input_ids.shape[1]:
                new_token_embeds = self.get_input_embeddings()(
                    input_ids[:, inputs_embeds.shape[1] :]
                )
