...
All arguments can be specified via YAML config, CLI flags, or both (CLI overrides YAML). The same config is shared across `quantize.py` and `train.py`. See [Advanced Topics](#advanced-topics).
...
A single YAML config can be shared across `quantize.py` and `train.py` — each script applies the arguments it recognizes and silently skips those belonging to other ModelOpt argument classes. Truly unknown keys (e.g., typos) will raise an error. All arguments can be specified via YAML config, CLI flags, or both (CLI overrides YAML). See [ARGUMENTS.md](ARGUMENTS.md) for the full reference.
...
Training configs embed a `dataset_config` path (default: `configs/dataset/blend.yaml`). See [Dataset Configuration](configs/dataset/README.md) for custom dataset blends, pre-tokenization, and adding new datasets.
